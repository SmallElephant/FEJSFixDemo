...
+ (void)fix {
    ...
    context[@"fixMethod"] = ^(NSString *instanceName, NSString *selectorName, FixOptions options, BOOL isClassMethod, JSValue *fixImpl) {
        [self fixMethod:isClassMethod options:options instanceName:instanceName selectorName:selectorName fixImp:fixImpl];
    };
...
+ (void)runJavaScript:(NSString *)jsString {
    if (jsString == nil || jsString == (id)[NSNull null] || ![jsString isKindOfClass:[NSString class]]) {
        return;
    }
...
+ (void)fixMethod:(BOOL)isClassMethod options:(FixOptions)options instanceName:(NSString *)instanceName selectorName:(NSString *)selectorName fixImp:(JSValue *)fixImpl {
    Class kClass = NSClassFromString(instanceName);
    if (isClassMethod) {
        kClass = object_getClass(kClass);
    }
    SEL sel = NSSelectorFromString(selectorName);
    [kClass aspect_hookSelector:sel withOptions:(AspectOptions)options usingBlock:^(id<AspectInfo> aspectInfo) {
